...
public class SignModel {
	...
	public SignDto modifySign(SignDto member){
		...
		SignDto obb =   member;
		SqlSession sqlSession = factory.openSession();
		int result = sqlSession.update("modifySign",obb);       // sqlSession 을 불러온 DB를 쓸건데 거기서 selectList 쓴다. mapper 파일에서 태그 이름 !  //sql실행후 결과값이 0,1 이여서  int로 되어있음
		sqlSession.commit();
	//	System.out.println("실행 result : "+result+"입력 결과는"+obb.getGB_CONTENTS()+" "+obb.getGB_SUBJECT()+" "+obb.getGB_ID()+" "+obb.getGB_FILE());
		sqlSession.close();
		return obb;
	}
	public SignDto modifyPw(SignDto member){ //입력된 아이디와 비밀번호
	...
	public SignDto deleteSangpum(SignDto sangpum){
