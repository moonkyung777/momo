				...
				<div class="container" id="formOutterWrapper">
					<center>
						...
						<div class="container" id="formInnerWrapper">
							...
							<table>
								<tr
									style="padding: 15px 15px 22px 15px; border-top: 2px solid #2c3c57;">

									<td class="viewform" colspan="2"
										style="font-size: 55px; padding-left: 40px;"><%=article.getGB_SUBJECT()%></td>
								</tr>
								<tr
									style="padding: 15px 15px 22px 15px; border-bottom: 1px solid #d5d8dd; padding-left: 40px;">
									<td class="viewsubject"><%=article.getGB_DATE()%></td>
									<td class="viewform"><%=article.getGB_ID()%></td>
								</tr>
								<tr>
									<td rowspan="2" class="viewsubject"
										style="border-bottom: 2px solid #2c3c57;">내용</td>
									<td class="viewform" style="padding-top: 50px;">
										<%
											if (!(article.getGB_FILE() == null)) {
										%> <a
										href="gal_file_down.jsp?downFile=<%=article.getGB_FILE()%>">
											<img alt="" src="boardUpload/<%=article.getGB_FILE()%>"
											class="imgst">
									</a> <%
 	}
 %>
								...
								<tr style="border-bottom: 2px solid #2c3c57;">
									...
									<td class="viewform" style="height: 100px;"><%=article.getGB_CONTENTS()%>
									</td>
								</tr>
							</table>

							<form name="galRepInsert" class="form" method="post"
								action="galRepPro.bo" role="form" autocomplete="off"
								enctype="multipart/form-data">
								<div id="rep">

									<input type="text" name="GB_REP_CONTENTS" id="GB_REP_CONTENTS" />
									<input type="submit" id="repSubmit"
										class="signbutton signbutton5" value="답글" /> <input
										type="hidden" name="GB_NUM" value="<%=article.getGB_NUM()%>"
										style="position: fixed;" /> <input type="hidden" name="GB_ID"
										value="<%=article.getGB_ID()%>" style="position: fixed;" />
						...
						<section id="commandList">
							<a
								href="galReplyForm.bo?GB_NUM=<%=article.getGB_NUM()%>&page=<%=nowPage%>">
								[답변] </a> <a href="galModifyForm.bo?GB_NUM=<%=article.getGB_NUM()%>">
								[수정] </a> <a
								href="galDeleteForm.bo?GB_NUM=<%=article.getGB_NUM()%>&page=<%=nowPage%>">
								[삭제] </a> <a href="galList.bo?page=<%=nowPage%>">[목록]</a>&nbsp;&nbsp;
						</section>
					</center>
